...
impl<S: Storage> NetTx<S> for &StaticLoopback<S> {
    ...
    fn fill_neighbor_cache(&mut self, _: Instant, _: (IpAddr, HwAddr), _: NeighborCacheOption) {}

    fn lookup_neighbor_cache(&self, _: Instant, ip: IpAddr) -> Result<HwAddr, NeighborLookupError> {
        if self.is_same_net(ip) {
            Ok(HwAddr::Ip)
        } else {
            Err(NeighborLookupError { rate_limited: false })
        }
    }
