//! `provide_any`, a generic API for objects to provide type-based access to
//! data. Copied from <https://github.com/nrc/provide-any>.
//!
//! `provide_any` supports generic, type-driven access to data and a mechanism
//! for intermediate implementers to provide such data. The key parts of
//! `provide_any`'s interface are the `Provider` trait for objects which can
//! provide data, and the `request_by_type_tag` function for requesting
//! data from an object which implements `Provider`. Note that end users should
//! not call `request_by_type_tag` directly, it is a helper function for
//! intermediate implementers to use.
//!
//! Typically, a data provider is a trait object of a trait which extends
//! `Provider`. A user will request data from the trait object by specifying the
//! type or a type tag (a type tag is a type used only as a type parameter to
//! identify the type which the user wants to receive).
//!
//! ```no-build
...
//! ```
//!
//! In this example obj is a trait object of some trait which has `request_ref`
//! and `request_by_type_tag` methods, both are implemented using
//! [`provide_any::request_by_type_tag`]. That trait will also extend
//! `Provider`.
//!
//! ## Data flow
//!
...
//!   `Provider::provide`
//! * The object provider's implementation of `Provider::provide` tries
//!   providing values of different types using `Requisition::provide_*`. If the
//!   type tag matches the type requested by the user, it will be stored in the
//!   `Requisition` object.
//! * `request_by_type_tag` unpacks the `Requisition` object and returns any
...
pub trait Provider {
    /// Object providers should implement this method to provide *all* values
    /// they are able to provide using `req`.
...
macro_rules! impl_provider_for_tuples {
    (@IMPL $($i:ident)*) => {
        #[allow(non_snake_case)]
        impl<$($i: 'static),*> Provider for ($($i,)*) {
            fn provide<'a>(&'a self, req: &mut Demand<'a>) {
                let ($($i,)*) = self;
                $(req.provide_ref($i);)*
            }
    ...
    ($first:ident $(, $next:ident)*) => {
        impl_provider_for_tuples!(@IMPL $first $($next)*);
        impl_provider_for_tuples!($($next),*);
    };
}

impl_provider_for_tuples!(A, B, C, D, E, F, G, H, I, J, K, L);

impl<const N: usize> Provider for [&dyn Provider; N] {
    fn provide<'a>(&'a self, req: &mut Demand<'a>) {
        for p in self {
            p.provide(req);
        }
...
impl<const N: usize> Provider for [&mut dyn Provider; N] {
    fn provide<'a>(&'a self, req: &mut Demand<'a>) {
        for p in self {
            p.provide(req);
        }
...
mod tags {
    ...
    //!
    //! Many users of [`Provided`] will not need to use type tags at all. But if
    //! you want to use [`Provided`] with more complex types (typically
...
impl<'a> Demand<'a> {
    ...
    /// Provide a value with the given `TypeTag`, using a closure to prevent
    /// unnecessary work.
    ...
    pub fn with<C>(&mut self, cx: C) -> DemandWith<'a, '_, C> {
        DemandWith { cx: Some(cx), demand: self }
    }
}
...
/// additional information about the specific `TypeTag` into the type. This
/// allows for multiple different tags to support overlapping value ranges, for
...
impl<'a> dyn Erased<'a> {
        ...
        I: tags::Type<'a>,
    {
        if self.tag_id() == TypeId::of::<I>() {
            ...
            Some(unsafe { &mut *(ptr::from_mut(self).cast::<TaggedOption<'a, I>>()) })
        } else {
            None
        }
    }
